<script lang="ts">
    ...
    async function getRentRequests() {
        try {
            const response = await fetch(config.host + '/candidates/' + getUser().id, {
                method: 'GET',
                headers: {
                  ...
                  <td class="h-px w-px whitespace-nowrap">
                    <div class="pl-6 lg:pl-3 xl:pl-0 pr-6 py-3">
                      <div class="flex items-center gap-x-3">
                        <!-- <img class="inline-block h-[2.375rem] w-[2.375rem] rounded-full" src="{btoa(candidate.photo)}" alt="Empty"> -->
