<script lang="ts">
    ...
    function validate() {
        if (username == null || password == null || username.length < 8 || password.length < 8) {
            error = true
            errorMessage = 'Fields shoud be min 8 characters'
            return false;
        }
        error = false
        return true;
...
<div class="h-[calc(100vh-61px)] bg-gradient-to-tl from-green-400 to-indigo-900 w-full py-16 px-4">
    <div class="flex flex-col items-center justify-center">
        <div class="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-5 mt-16">
            <p class="focus:outline-none text-2xl font-extrabold leading-6 text-gray-800">Create account</p>
            ...
            <input bind:value={username} aria-labelledby="username" type="username" class="bg-gray-200 border rounded  text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2"/>
                ...
                <div class="mt-8">
                    <button on:click={signup} class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full">Sign up</button>
                </div>
                
                <p class="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-500">Already have account? 
                    <a href="/signin" class="hover:text-gray-500 focus:text-gray-500 focus:outline-none focus:underline hover:underline text-sm font-medium leading-none  text-gray-800 cursor-pointer"> Sign in here</a>
                </p>
        </div>
    </div>
</div>
