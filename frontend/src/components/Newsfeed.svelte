<script lang="ts">
    ...
    async function getPosts() {
        try {
            const response = await fetch(config.host + '/posts/subscriptions', {
                method: 'GET',
                headers: {
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-gray-700">
                    <div class="px-6 py-4 grid gap-3  md:items-center border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="border-b border-gray-900/10 pb-12">
                            <h2 class="text-xl text-base font-semibold leading-7 text-gray-900">Posts</h2>

                            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-3">
                                {#if posts.length == 0}
                                    <h2 class="text-xl text-base font-semibold leading-7 text-gray-900">No posts yet</h2>
                                {/if}
                                {#each posts as post}
                                    <div class="sm:col-span-1">
                                        <label for="name" class="block text-sm font-medium leading-6 text-gray-900">Hero</label>
                                        <div class="mt-2">
                                            <input disabled bind:value={post.heroName} type="text" name="name" id="name" autocomplete="family-name" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                        </div>
                                    </div>
