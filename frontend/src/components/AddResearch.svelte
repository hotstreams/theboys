<script lang="ts">
    ...
    import { addResearch } from './Reseaches';
    ...
    function validate() {
        if (selectedCandidate == null || selectedMedicine  == null || selectedMedicine.dozeMl  == null || selectedMedicine.dozeCount == null) {
            error = true
            errorMessage = 'All fields should be selected'
            return false
        }
        return true
    }
    ...
    async function add() {
        ...
        const res = await addResearch(selectedCandidate.id, selectedMedicine.id, selectedMedicine.dozeMl, selectedMedicine.dozeCount)
        if (res.error) {
            error = true
...
<CandidateList bind:selectedCandidate={selectedCandidate}/>

<MedicineList bind:selectedMedicine={selectedMedicine}/>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900">
                    <div class="px-6 py-4 grid gap-3 md:flex justify-end md:items-center border-b border-gray-200">
                        ...
                        <div class="px-3 py-1.0">
                            <a on:click={add} class="py-2 px-3 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800" href="#">
                                Add research
                            </a>
                        </div>
                    </div>
